...
public class Zad4 {
	...
	public static void main(String[] args) {
		...
		do{	
			...
			System.out.println("\tWcisnij \"4\" aby wyswietlić wszystkie przdmioty");
			System.out.println("\tWcisnij \"9\" aby Zakończyć program." );
			...
			case 1:{
				...
				do{
					...
					for (int i = 0; i < inVoice.size();i++){
						if(inVoice.get(i).ID == parmID){
							System.out.println("Jest już produkt z takim ID\nPodaj inne ID:");
							ifID=true;	break;
						}
					}
				...
				System.out.println("Wprowadz cene za sztukać.");
			...
			case 2:{
				...
				do{
					...
					for (int i = 0; i < inVoice.size();i++){
						if(inVoice.get(i).ID == printProduct){
							...
							System.out.println("Nie ma przedmiotu z takim ID, wybierz prawidłowe.");						
						}
					}
				}while(!ifID);	ifID=true;
				...
				System.out.println("Wartość wszystkich produktów: " + inVoice.get(printProduct).getInVoiceAmount(inVoice.get(printProduct).getAmount(),inVoice.get(printProduct).getPrice()));
				break;
			}
			case 3:{
				System.out.println("Wybierz ID przedmiotu który chcesz usunąć");
			...
			case 4:{
				for(int i=0;i<inVoice.size();i++){
					...
					System.out.println("Wartość wszystkich przedmiotów o tym ID to: " + inVoice.get(i).getInVoiceAmount(inVoice.get(i).getAmount(),inVoice.get(i).getPrice()) + "\n");
			...
			default:{
				System.out.println("Zły wybór, popraw się!");
				break;
			}
			}
		}while(exit);			
	...
	public static int ScannerInt(){
		...
		do{	
			try{
				...
				System.out.println("Podany argument nie jest liczbę.\nPodaj jeszcze raz.");
				scanner.next();
			}
		}while(true);
